...
namespace USBRHarvester
{
    class RISE_ApiManager
    {
        ...
        public async Task<List<USBRCatalogItem.Data>> GetCatalogItemsAsync(List<USBRCatalogRecord.Data> catalogRecords)
        {
            //GetCatalogItems from API that are timeseries "itemstructureId=1, Geospatial=2, FileUpload=3" max per page = 100
            //TODO:iterate through pages
                        ...
                        .ContinueWith(async (itemSearchTask) =>
                        {
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                if (result != null )
                                {
                                    ...
                                    _log.LogWrite(result.data.attributes._id +","+ result.data.attributes.locationName);
        ...
        public void GetCatalogItems()
        {
            ...
            try
            {
                using (WebClient webClient = new WebClient())
                {
                    ...
                    var jsonData = webClient.DownloadString("catalog-record?itemStructureId=1&hasItemStructure=true&itemsPerPage=100");
                    USBRCatalogitemRoot USBRcatalogRecords = JsonConvert.DeserializeObject<USBRCatalogitemRoot>(jsonData);
                    _log.LogWrite(String.Format("Found {0} distinct USBRCatalogRecord.", USBRcatalogRecords.data.Count));
                }
            }
