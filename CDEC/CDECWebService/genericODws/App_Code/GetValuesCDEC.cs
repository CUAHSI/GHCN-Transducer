...
namespace WaterOneFlow.odws
{
    ...
    public class GetValuesCDEC
    {
        ...
        public static TimeSeriesResponseType GetValues(string SiteNumber, string Variable, string StartDate, string EndDate)
        {
            ...
            resp.timeSeries[0].variable = GetVariableInfoFromDb(variableCode);            
            resp.timeSeries[0].values = new TsValuesSingleVariableType[1];            
            resp.timeSeries[0].values[0] = GetValuesFromDb(siteId, variableCode, startDateTime, endDateTime);
            //set the query info
        ...
        internal static TsValuesSingleVariableType GetValuesFromDb(string siteCode, string variableCode, DateTime startDateTime, DateTime endDateTime)
        {
            ...
            var baseDataUrl = "https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet";
        //https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=PSR&SensorNums=18&dur_code=D&Start=2021-06-07&End=2021-07-07
            ...
            var dataUrl = String.Format("{0}?Stations={1}&SensorNums={2}&dur_code={3}&Start={4}&End={5}",
                baseDataUrl,
                siteCode,
                v.variableCode[0].variableID, 
                "d",
            ...
            try
            {
                using (var stream = client.OpenRead(dataUrl))
                {
                    using (var reader = new StreamReader(stream))
                    {
                        ...
                        {
                            csv.Context.RegisterClassMap<CDECValueResponseModelClassMap>();
                            var values = csv.GetRecords<CDECValueResponseModel>().ToList();

                            foreach (var item in values)
                            {
                                var format = "yyyyMMdd HHmm";
                                CultureInfo provider = CultureInfo.InvariantCulture;
                                var PST_dt = DateTime.ParseExact(item.OBSDATE, format, provider); //is in PST according to website https://cdec.water.ca.gov/dynamicapp/wsSensorData
                                TimeZoneInfo tst = TimeZoneInfo.FindSystemTimeZoneById("Pacific Standard Time");
                                var UTC_dt = TimeZoneInfo.ConvertTimeToUtc(PST_dt, tst);

                                var dataValueStr = item.VALUE.ToString();
                                ...
                                newValue.dateTimeUTC = UTC_dt;
                                newValue.dateTimeUTCSpecified = true;
                                newValue.timeOffset = "-8";
